...
reverse, and it holds on two instruments at once.** A 29 minute capture
carrying BTC and ETH quoted by both venues confirms the ordering separately
on each, under identical market conditions: BTC at z = 5.19 and ETH at
z = 9.43, with Binance's moves answered roughly four times as often as
Coinbase's on both. The second instrument is a control on the first rather
than a separate study.

It replicates across sessions. Across two earlier 45 minute captures,
Binance was answered 0.577 of the time against Coinbase's 0.264
(z = 11.76, 204,864 messages) and 0.903 against 0.539 in a session eight
days later that was three times as busy (z = 39.33, 642,919 messages). The
ordering holds at every threshold from $0.25 to $2.00 in both, and the
measurement is biased *against* that conclusion by roughly 36 to 86 ms of
network asymmetry.
...
significance, not a percentage.

**A third estimator says why the other two could only ever report an
ordering.** Hayashi-Yoshida is the standard covariance estimator for prices
observed at different, irregular times: it pairs returns over overlapping
observation intervals instead of resampling both venues onto a common grid.
On the same two captures it finds co-movement of 0.71 and 1.18 where the
grid estimator found 0.25 and 0.31, so the grid was recovering roughly a
quarter to a third of what is in these books and discarding the rest as
synchronization bias. That gap replicates. The millisecond-scale
...
Both sockets are read by one process so the streams share a clock, which is
what makes the measurement possible at all. Two processes, or two clocks,
would put an unknown offset directly into the quantity being estimated.
...
- [`docs/postmortems.md`](docs/postmortems.md) - five defects that survived
  their own test suites, and why nothing caught them
- [`docs/analytics_notes.md`](docs/analytics_notes.md) - what the analytics
  compute, and the failures that shaped them
- [`docs/design.md`](docs/design.md) - how the code is put together
- [`docs/bench/`](docs/bench/) - one writeup per measurement, each with the
...
that shaped them: [`docs/analytics_notes.md`](docs/analytics_notes.md).
