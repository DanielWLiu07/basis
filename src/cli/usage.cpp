...
int usage() {
  std::printf(
      ...
      "      --speed N replays at the capture's own arrival schedule,\n"
      "                compressed N times (1 = real time, 0 = flat out).\n"
      "                Without it the harness can only report SERVICE\n"
      "                time: a slow record delays its successors and\n"
      "                nothing measures the delay. Pacing adds response\n"
      "                time, measured from when each record was due;\n"
      "      --pace-spin-ms MS how much of each wait is spun rather than\n"
      "                slept (default 2). A sleep overshoots by\n"
      "                milliseconds under load, which swamps a\n"
      "                microsecond response time; past the capture's\n"
      "                largest gap this spins every wait and pins a core;\n"
      ...
      "      --seconds or ctrl-c (default config: contracts-live.toml, the\n"
      "      one meant for new recordings; check it is still tradeable with\n"
      "      scripts/contracts.py check).\n"
