...
struct WsConfig {
  ...
  // Extra HTTP headers for the upgrade request (venue auth goes here).
...
// send() is only valid from inside on_connect (before the read loop owns
// the stream). stop() is safe from any thread and joins the IO thread.
class WsClient {
 public:
